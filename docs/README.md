...
| Keyboard backlight | Working              | With [`SSDT-KBBL.aml`](acpi/SSDT-KBBL.dsl)                                                    |           
| Keyboard & Remaps  | Working              | See remap ACPI sample [here](acpi/ssdt-chromebook-keys.dsl)                                   |
...
* Method 1 (recommended by isi95010): In an SSDT, set _STA to 0 on all CPU threads, and then define new CPU thread names with compatible addressing and plugin-type set. See [4-thread CPU sample SSDT](acpi/ssdt-plug-4200.dsl) that you can compile and use. 
...
11. You can use the standard VoodooPS2controller and VoodooPS2keyboard plugin with the [PS2 chromebook remapping SSDT sample](acpi/ssdt-chromebook-keys.dsl). 
   - Keyboard backlight works with `SSDT-KBBL.aml` and can be found [here](acpi/SSDT-KBBL.dsl).
...
- Method 1 Rename the CPU threads while adding CPU addressing and plugin-type with [this SSDT](acpi/ssdt-plug-4200.dsl). 
